...
while step < total_steps:
  ...
  policy.eval()
  for _ in range(num_steps): # for each step we evaluate the baseline (old) policy
    # Use policy
  ...
  for epoch in range(num_epochs): # for each epoch we update the new policy
    ...
    for batch in generator: 
      ...
      # entropy controls the distrubtion variance for exploitation
  ...
  exp_name_save = exp_name + '_' + str(step)
  ...
  if step >= total_steps:
    ...
    torch.save(checkpoint, 'checkpoint_completed_nature_'+exp_name_save+'.pth')
  else:
    checkpoint = { 
    'step': step,
    'step list': step_list,
    ...
    'action': action,
    'obs test': obsTest}
    torch.save(checkpoint, 'checkpoint_nature_'+exp_name_save+'.pth')

#print('Completed training!')
#torch.save(policy.state_dict(), 'checkpoint_'+exp_name+'.pt')

##%
#torch.save(policy, 'checkpoint_model_'+exp_name+'.pt')
...
# imageio.mimsave('vid_nobackground_'+exp_name+'.mp4', frames, fps=25)

# # with background
# # Make evaluation environment
# eval_env = make_env(num_envs, env_name, start_level=num_levels, num_levels=num_levels, use_backgrounds=True)
# obs = eval_env.reset()
...
# # Save frames as video
# frames = torch.stack(frames)
# imageio.mimsave('vid_background_'+exp_name+'.mp4', frames, fps=25)
